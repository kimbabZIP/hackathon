...
| 강의 음성 분석 | `POST /api/audio/analyze` |
| 교수 대화 | `POST /api/chat` |
...
강의 음성 분석은 `scholarly-affection/transcript.txt`가 존재하고 내용이 있을 때 기존 데모 전문을 우선 사용합니다. 파일이 없거나 비어 있으면 업로드 음성의 앞 5분(300초)만 16kHz 모노 WAV로 잘라 Gemini Files API에 임시 업로드하며, 5분 이후 구간은 사용하지 않습니다. 긴 교수 음성 전문은 JSON이 아닌 일반 텍스트로 먼저 추출하고, 그 전문을 별도의 구조화 호출에 전달해 강의 요약과 교수 말투 특징을 생성합니다. 분석 후 Gemini에 올린 임시 오디오와 서버의 원본·변환 임시 파일은 삭제됩니다. 실제 분석 경로에는 `GEMINI_API_KEY` 또는 `GOOGLE_API_KEY`가 필요합니다.

음성 처리 진행 상황은 `npm run api`를 실행한 터미널에 `[audio:요청ID] [1/6]` 형식으로 출력됩니다. 업로드, Gemini 파일 준비, STT·화자 분리·요약 분석, 결과 검증, 임시 파일 정리, 데이터베이스 저장 순서를 확인할 수 있고 LLM 응답을 기다리는 동안에는 15초마다 진행 중 로그가 출력됩니다.
...
- `assignment_grader/lecture_audio.py`: 고정 transcript 우선 및 Gemini 음성 STT·요약 폴백

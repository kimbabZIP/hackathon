...
강의 음성 분석은 `scholarly-affection/transcript.txt`가 존재하고 내용이 있을 때 기존 데모 전문을 우선 사용합니다. 파일이 없거나 비어 있으면 업로드한 오디오를 Gemini Files API에 임시 업로드합니다. 긴 교수 음성 전문은 JSON이 아닌 일반 텍스트로 먼저 추출하고, 그 전문을 별도의 구조화 호출에 전달해 강의 요약과 교수 말투 특징을 생성합니다. 분석 후 Gemini에 올린 임시 오디오와 서버의 임시 파일은 삭제됩니다. 실제 분석 경로에는 `GEMINI_API_KEY` 또는 `GOOGLE_API_KEY`가 필요합니다.
